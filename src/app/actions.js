...
export const addNewCampaign = (campaignToAdd) => async(dispatch) => {
  ...
  try {
    ...
    let finalCampaignList = checkIfUserExistForGivenCampaign(campaignToAdd, userList);
    dispatch(updateCampaigns(finalCampaignList));
    dispatch(setLoading(false));
  } catch (error) {
    console.error('Some error occured while adding a new campaign. Please reload and try again.');
...
export const getUsers = async () => {
  ...
  try {
    ...
    console.error('Some error occured while fetching user list. Please reload and try again.')
    return error;
  }
};
